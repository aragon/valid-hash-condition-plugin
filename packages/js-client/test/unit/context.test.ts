import { ValidHashConditionContext } from '../../src';
import { ValidHashConditionContextParams } from '../../src/types';
...
describe('Context instances', () => {
  let contextParams: ValidHashConditionContextParams;
  const TEST_WALLET =
  ...
  beforeEach(() => {
    ...
    contextParams = {
      ...
      ValidHashConditionPluginAddress: 'notDefault',
    };
  });
  it('Should create an empty context and have default values', () => {
    const context = new ValidHashConditionContext();
    expect(context).toBeInstanceOf(ValidHashConditionContext);
    ...
    expect(context.ValidHashConditionPluginAddress).toBe(
  ...
  it('Should create a context and have the correct values', () => {
    const context = new ValidHashConditionContext(contextParams);
    ...
    expect(context.ValidHashConditionPluginAddress).toBe('notDefault');
  });
  ...
  it('Should create a context in goerli, update the network and update all the parameters automatically', () => {
    ...
    context.set({
      ...
      ValidHashConditionPluginAddress: 'otherValue',
    });
    ...
    expect(context.ValidHashConditionPluginAddress).toBe('otherValue');
  });
  ...
  it('Should create a context with invalid network and fail', () => {
    ...
    expect(() => {
      new ValidHashConditionContext(contextParams);
    }).toThrow();
  });
  ...
  it('it should use a context to initialize the ValidHashConditionContext', () => {
    const context = new Context(contextParams);
    const ValidHashConditionContext = new ValidHashConditionContext(
      {
        ValidHashConditionPluginAddress: 'notDefault',
      },
      context
    );
    expect(ValidHashConditionContext.network.name).toBe('homestead');
    expect(ValidHashConditionContext.network.chainId).toBe(1);
    expect(ValidHashConditionContext.daoFactoryAddress).toBe(
      contextParams.daoFactoryAddress
    );
    expect(ValidHashConditionContext.ensRegistryAddress).toBe(
      ValidHashConditionContext.network.ensAddress
    );
    expect(ValidHashConditionContext.gasFeeEstimationFactor).toBe(
      contextParams.gasFeeEstimationFactor
    );
    ValidHashConditionContext.web3Providers.map((provider) =>
      expect(provider).toBeInstanceOf(JsonRpcProvider)
    );
    ValidHashConditionContext.ipfs.map((ipfsClient) =>
      expect(ipfsClient).toBeInstanceOf(IpfsClient)
    );
    ValidHashConditionContext.graphql.map((graphqlClient) =>
      expect(graphqlClient).toBeInstanceOf(GraphQLClient)
    );
    expect(ValidHashConditionContext.ValidHashConditionPluginAddress).toBe('notDefault');
  });
});
