...
export async function buildValidHashConditionDao(deployment: Deployment) {
  try {
    const latestVersion = await deployment.ValidHashConditionRepo[
      'getLatestVersion(address)'
    ](deployment.ValidHashConditionSetup.address);
    return await createDao(
      ...
      [
        {
          pluginSetupRef: {
            pluginSetupRepo: deployment.ValidHashConditionRepo.address,
