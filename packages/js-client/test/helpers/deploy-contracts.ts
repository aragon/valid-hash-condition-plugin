...
export type Deployment = OsxDeployment & ValidHashConditionDeployment & EnsDeployment;

export type ValidHashConditionDeployment = {
  ValidHashConditionRepo: aragonContracts.PluginRepo;
  ValidHashConditionSetup: ValidHashConditionSetup;
};
...
export async function deploy(): Promise<Deployment> {
  ...
  const simpleSotrage = await deployValidHashConditionContracts(deployOwnerWallet, osx);
...
export async function deployValidHashConditionContracts(
  deployer: Signer,
  osx: OsxDeployment
): Promise<ValidHashConditionDeployment> {
  const ValidHashConditionFactory = new ValidHashConditionSetup__factory();
  const ValidHashConditionPluginSetup = await ValidHashConditionFactory.connect(deployer).deploy();

  const ValidHashConditionRepoAddress = await deployPlugin(
    'simple-storage',
    ValidHashConditionPluginSetup.address,
    await deployer.getAddress(),
    osx.pluginRepoFactory
  );

  const ValidHashConditionRepo = aragonContracts.PluginRepo__factory.connect(
    ValidHashConditionRepoAddress,
    deployer
  );

  return {
    ValidHashConditionRepo,
    ValidHashConditionSetup: ValidHashConditionPluginSetup,
  };
}
